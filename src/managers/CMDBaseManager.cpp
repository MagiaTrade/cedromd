...
namespace cedro::md
{
                                      ...
                                      bool snapshot)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] Failed to send SQT message";
        logE << (errorMsg);
      ...
      else
      {
        ...
        logI << "[CMDBaseManager]  Subscribed successfully";
      }
    });
  }
  ...
  void CMDBaseManager::unsubscribeQuote(const std::string& symbol, const SuccessCallback& cb)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        logE << "[CMDBaseManager] Failed to send USQ message";
        cb(false);
      }
      else
      {
        logI << "[CMDBaseManager] Unsubscribed successfully";
  ...
  void CMDBaseManager::subscribeBookQuote(const std::string& symbol, const StreamCallback& cb)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] Failed to send BQT message";
        logE << (errorMsg);
        cb(false, errorMsg.c_str(), errorMsg.length());
      }
      else
      {
        ...
        logI << ("[CMDBaseManager] Subscribed to book quote successfully");
      }
    });
  }
  ...
  void CMDBaseManager::unsubscribeBookQuote(const std::string& symbol, const SuccessCallback& cb)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] Failed to send UBQ message");
        cb(false);
      }
      else
      {
        logI << ("[CMDBaseManager] Unsubscribed from book quote successfully");
                                      ...
                                      bool snapshot)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] Failed to send SAB message";
        logE << (errorMsg);
        cb(false, errorMsg.c_str(), errorMsg.length());
      }
      else
      {
        ...
        logI << ("[CMDBaseManager] SAB - Subscribed successfully");
      }
    });
  }
  ...
  void CMDBaseManager::unsubscribeAggBook(const std::string& symbol, const SuccessCallback& cb)
  {
    ...
    [&, symbol, cb](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] Failed to send UAB message");
        cb(false);
      }
      else
      {
        logI << ("[CMDBaseManager] UAB - Unsubscribed successfully");
  ...
  void CMDBaseManager::subscribeQuoteTrades(const std::string &symbol, const StreamCallback &cb)
  {
    ...
    [&, cb, symbol](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] Failed to send GQT message";
        logE << (errorMsg);
        cb(false, errorMsg.c_str(), errorMsg.length());
      }
      else
      {
        ...
        logI << ("[CMDBaseManager] GQT - Subscribed successfully");
      }
    });
  }
                                           ...
                                           size_t offset)
  {
    ...
    [&, cb, requestId](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] GQT(snap): Failed to send message";
        logE << (errorMsg);
      ...
      else
      {
        ...
        logI << ("[CMDBaseManager] GQT(snap): Subscribed successfully");
      }
    });
  }
  ...
  void CMDBaseManager::unsubscribeQuoteTrades(const std::string &symbol, const SuccessCallback& cb)
  {
    ...
    [&, symbol, cb](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] UQT: Failed to send UAB message");
        cb(false);
      }
      else
      {
        logI << ("[CMDBaseManager] UQT: Unsubscribed successfully");
                                      ...
                                      const StreamWithEndFlagCallback& cb)
  {
    ...
    [&, cb, marketName](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] Failed to send GPN message";
        logE << (errorMsg);
        cb(false, errorMsg.c_str(), errorMsg.length(), true);
      }
      else
      {
        ...
        logI << ("[CMDBaseManager] Requested player names successfully");
      }
    });
  }
                                      ...
                                      const VAPRequestInfo& info)
  {
    ...
    [&, symbol, cb, info](bool success)
    {
      if (!success)
      {
        std::string errorMsg = "[CMDBaseManager] VAP(snap): Failed to send message";
        logE << (errorMsg);
      ...
      else
      {
        ...
        logI << ("[CMDBaseManager] VAP(snap): Subscribed successfully");
      }
    });
  }
                                        ...
                                        int32_t lastMinutes)
  {
    ...
    {
      std::stringstream stm;
      stm << "[CMDBaseManager] VAP minutes: " << lastMinutes << " not supported. Please use one of these values: [1, 2, 5, 10, 15, 30 , 60]";
      auto errmsg = stm.str();
      logW << (errmsg);
                                    ...
                                    bool accumulated)
  {
    ...
    {
      std::stringstream stm;
      stm << "[CMDBaseManager] VAP history: missing date. Please set a valid date range!";
      auto errmsg = stm.str();
      logW << (errmsg);
      cb(false, errmsg.c_str(), errmsg.length(), true, info.type);
      return;
  ...
  void CMDBaseManager::connect(const ConnectionCallback &cb)
  {
    if(_isConnected)
    {
      logW << ("[CMDBaseManager] Already connected!");
      cb(true, _stream);
      return;
    }

    switchURL = !switchURL;
    auto url = _shouldUseTestURL ? FEED_URL_TESTS :  (switchURL ? FEED_URL_1 : FEED_URL_2);
    auto portNumber = _shouldUseTestURL ? "1234" : PORT_NUMBER;

    auto stream = _streamer->openStream(url, portNumber, "",
  ...
  {
    logI << ("[CMDBaseManager] disconnect called!");

    if(!_isConnected)
    {
      logW << "[CMDBaseManager] Already disconnected!";
      if(cb)
        cb(false);
      return;
    }
  ...
  void CMDBaseManager::handleMessage(const char* data, size_t size)
  {
    ...
    {
      logD << ("[CMDBaseManager] SYN: Sync message received!");
      if(_heartBeatCallback)
        _heartBeatCallback();
    }
    else
    {
      logW << ("[CMDBaseManager] Unhandled message type received: " + std::string(data, size));
    }
  }
  ...
  void CMDBaseManager::handleSQTMessage(const char* data, size_t size)
  {
    ...
    if (symbol.empty())
    {
      logW << ("[CMDBaseManager] Could not extract symbol for SQT");
      return;
    }
  ...
  void CMDBaseManager::handleBQTMessage(const char* data, size_t size)
  {
    ...
    if (symbol.empty())
    {
      logW << ("[CMDBaseManager] Could not extract symbol for BQT");
      return;
    }
    ...
    {
      logW << ("[CMDBaseManager] BQT message size exceeds buffer capacity. Ignoring it!");
      return;
    }
  ...
  void CMDBaseManager::handleSABMessage(const char* data, size_t size)
  {
    ...
    if (symbol.empty())
    {
      logW << ("[CMDBaseManager] Could not extract symbol for SAB");
      return;
    }
    ...
    {
      logW << ("[CMDBaseManager] SAB message size exceeds buffer capacity. Ignoring it!");
      return;
    }
  ...
  void CMDBaseManager::handleGQTSnapshotMessage(const char* data, size_t size, const std::string& requestId)
  {
    ...
    if (symbol.empty())
    {
      logW << ("[CMDBaseManager] GQT: Could not extract symbol");
      return;
    }
    ...
    {
      logW << ("[CMDBaseManager] GQT: message size exceeds buffer capacity. Ignoring it!");
      return;
    }
  ...
  void CMDBaseManager::handleGPNMessage(const char* data, size_t size)
  {
    ...
    if (market.empty())
    {
      logW << ("[CMDBaseManager] GPN: Could not extract market name.");
      return;
    }

    if(size + 1 > GPN_BUFFER_SIZE)
    {
      logW << ("[CMDBaseManager] GPN: message size exceeds buffer capacity. Ignoring it!");
      return;
    }
    ...
    {
      std::stringstream stm;
      stm << "[CMDBaseManager] GPN: callback no found for Market: " << market <<  std::string(data, size) << "\n";
      logW << (stm.str());
      return;
    }
  ...
  void CMDBaseManager::handleVAPSnapshotMessage(const char *data, size_t size)
  {
    ...
    if (symbol.empty())
    {
      logW << ("[CMDBaseManager] VAP(snap): Could not extract symbol");
      return;
    }

    if(size + 1 > VAP_BUFFER_SIZE)
    {
      logW << ("[CMDBaseManager] VAP(snap): message size exceeds buffer capacity. Ignoring it!");
      return;
    }
    ...
    {
      std::stringstream stm;
      stm << "[CMDBaseManager] VAP: callback no found for: "
          << info.symbol << " Type: "
          << info.type << "\n";
                                            ...
                                            const ConnectionCallback &cb)
  {
    ...
    {
      logI << ("[CMDBaseManager] Received empty or irrelevant message");
      return;
    }
    ...
    {
      logI << "[CMDBaseManager] You are connected received!";
      if(_stream)
        logI << "[CMDBaseManager] Stream ID " << _stream->getId() << " use count: " << _stream.use_count();
    ...
    {
      logI << "[CMDBaseManager] Useless message: " << data;
    }
  }
                                   ...
                                   const ConnectionCallback &cb) const
  {
    _messenger->sendMessage(stream, _softKey + "\n", [&, cb](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] Failed to send soft key");
        cb(false, nullptr);
      }
      else
      {
        logI << ("[CMDBaseManager] Soft key sent successfully");
      }
    });
  }
                                    ...
                                    const ConnectionCallback &cb) const
  {
    _messenger->sendMessage(stream, _username + "\n", [&,cb](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] Failed to send username");
        cb(false, nullptr);
      }
      else
      {
        logI << ("[CMDBaseManager] Username sent successfully");
      }
    });
  }
                                    ...
                                    const ConnectionCallback &cb) const
  {
    _messenger->sendMessage(stream, _password + "\n", [&, cb](bool success)
    {
      if (!success)
      {
        logE << ("[CMDBaseManager] Failed to send password");
        cb(false, nullptr);
      }
      else
      {
        logI << ("[CMDBaseManager] Password sent successfully");
      }
    });
  }
  ...
  bool CMDBaseManager::safetyCheck(const std::string& symbol)
  {
    ...
    if(!isConnected())
    {
      logE << ("[CMDBaseManager] Stream not connected!");
      return false;
    }

    if(symbol.empty())
    {
      logE << ("[CMDBaseManager] Symbol is empty!");
      return false;
    }
